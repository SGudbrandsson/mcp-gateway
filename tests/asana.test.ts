...
describe('Asana adapter', () => {
  ...
  describe('upload_attachment', () => {
    ...
    const tmpFile = join(tmpdir(), 'mcp-relay-kit-test-upload.png');
    ...
    it('uploads a file as multipart form data', async () => {
      mockFetch.mockResolvedValueOnce(mockAsanaResponse({ gid: '999', name: 'mcp-relay-kit-test-upload.png' }));
      const result = await action.execute({ task_id: '123', file_path: tmpFile }, config);
      expect(result).toEqual({ gid: '999', name: 'mcp-relay-kit-test-upload.png' });
